...
## Feature (2026/6/17) — 性能基准测试增加朴素矩阵乘法对比

### 修改内容

在 GEMM 性能基准测试中增加朴素（非 BLAS）矩阵乘法实现，与 OpenBLAS 进行直观对比。

1. **新增 `naive_gemm` 函数**：三层循环实现的朴素矩阵乘法（列优先存储），
   作为 OpenBLAS `cblas_dgemm` 的性能对比基准
2. **修改基准测试输出格式**：两列并排显示 OpenBLAS 和朴素实现的耗时，
   额外计算加速比，并逐元素比较两种实现的计算结果（相对容差 1e-8）

### 修改文件

| 文件                | 修改内容                                               |
|--------------------|--------------------------------------------------------|
| `include/blas_demo.h` | 添加 `naive_gemm` 函数声明                          |
| `src/blas_demo.cpp`   | 添加 `naive_gemm` 实现（三层循环，列优先）          |
| `src/main.cpp`        | 修改 `benchmark_gemm()`，同时运行两种实现并对比输出 |

### 预期输出格式

```
矩阵规模      | OpenBLAS (ms) | 朴素实现 (ms) | 加速比  | 一致
------------------------------------------------------------------------
   128 x  128  |         0.04  |         1.23  |  30.8x  |  YES
   256 x  256  |         0.20  |         9.87  |  49.4x  |  YES
   ...
```
